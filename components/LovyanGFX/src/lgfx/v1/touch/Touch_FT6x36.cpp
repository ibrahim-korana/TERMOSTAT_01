...
//----------------------------------------------------------------------------
  ...
  static constexpr uint8_t FT6x36_CIPHER_REG  = 0xA3;
  static constexpr uint8_t FT6x36_FIRMWARE_VERSION  = 0xA6;

  static constexpr uint8_t FT6x36_INTMODE_REG = 0xA4;
  static constexpr uint8_t FT6x36_POWER_REG   = 0xA5;
  static constexpr uint8_t FT6x36_VENDID_REG  = 0xA8;
  static constexpr uint8_t FT6x36_PERIODACTIVE = 0x88;

  static constexpr uint8_t FT6x36_PANEL_ID  = 0xA8;
  static constexpr uint8_t FT6x36_CHIP_ID  = 0xA3;
  static constexpr uint8_t FT6x36_FIRMWARE_ID  = 0xA6;


  static constexpr uint8_t FT6x36_MONITOR  = 0x01;
  static constexpr uint8_t FT6x36_SLEEP_IN = 0x03;


  bool Touch_FT6x36::_write_reg(uint8_t reg, uint8_t val)
  {
  ...
  bool Touch_FT6x36::_check_init(void)
  {
    ...
    _inited = _write_reg(0x00, 0x00)
          && _read_reg(FT6x36_CIPHER_REG, tmp, 6)
          && _write_reg(FT6x36_INTMODE_REG, 0x00) // INT Polling mode
          && tmp[5]
          ;
  printf("CIPHER:0x%02x / FIRMID:0x%02x / VENDID:0x%02x\n", tmp[0], tmp[3], tmp[5]);
...
if (_inited)
{
  ...
  void Touch_FT6x36::wakeup(void)
  {
    ...
    _write_reg(FT6x36_POWER_REG, FT6x36_MONITOR);
  }

  void Touch_FT6x36::sleep(void)
  {
    if (!_check_init()) return;
    _write_reg(FT6x36_POWER_REG, FT6x36_SLEEP_IN);
  }

  size_t Touch_FT6x36::_read_data(uint8_t* readdata)
  {
    /// 戻り値res: 通信に失敗した場合は0、通信に成功した場合はByte数

   // uint8_t ges;
   // _read_reg(0x01, &ges, 1);
  ...
  size_t Touch_FT6x36::_read_data1(uint8_t* readdata)
  {
    size_t res = 0xFF;

uint8_t pp[17];
    if (_read_reg(0xD3,pp,16))
    {
       res = 0;
       for (int k=0;k<16;k++) {
        printf("%02x ",pp[k]);
  ...
  uint_fast8_t Touch_FT6x36::getTouchRaw(touch_point_t *tp, uint_fast8_t count)
  {
